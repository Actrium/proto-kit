//! 验证项目核心功能：将多个 proto 文本按 package 分组并合并

use proto_fmt::merge::merge_by_package;
...
message Foo { string bar = 1; }"#;
    ...
    // 转换为元组格式: Vec<(String, String)>
    let tuple_format: Vec<(String, String)> = results
        .into_iter()
        .map(|r| (r.package_name, r.content))
        .collect();
    ...
    println!("\n✅ 元组格式转换验证通过！");
    println!("   格式: Vec<(String, String)>");
    println!("   内容: (package_name, content)");
}
...
fn test_wrapper_function() {
    // 演示如何包装成用户期望的签名
    fn merge_to_tuples(files: Vec<String>) -> anyhow::Result<Vec<(String, String)>> {
    ...
    println!("\n✅ 包装函数验证通过！");
    println!("   签名: fn(Vec<String>) -> Result<Vec<(String, String)>>");
}
